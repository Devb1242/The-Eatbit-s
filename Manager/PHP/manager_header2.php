<div class="srch_box">
    <div class="search-input">
        ...
        <div class=" autocom-box" id="auto">
            <?php
                    $a=array("add_product_ingredients.php","add_production.php","add_purchases.php","add_raw.php","help.php","index.php","product_ingredients.php","production.php","profile.php","purchase.php","raw_use.php","raw.php","view_category.php");                                                                                             
                    $b=array("Add Product Ingredients","Add Production","Add Purchase","Add Raw Materials","Help","Dashboard","Product Ingredients","Production","My Profile","Purchase","Raw Used","Raw Materials","Raw Category");
                ?>
    ...
    <div class="account" style="cursor:pointer" onclick="menuToggle()">
        <?php
			...
			echo "<div class='usrimg'><img src='images/manager image/".$res['m_img']."' /></div><b> ".$res['m_name']."</b>";
		?>
	...
	<?php
		$n_str="select * from info_sale where ord_status=2 and noti=0";
		$n_qry=mysqli_query($conn,$n_str);
		$n_count=mysqli_num_rows($n_qry);
		
		$f_str="select * from feedback where noti=0 and d_id<>''";
	...
	<div class="menu2" id="menu2">
        <ul>
			<?php
					...
					{
						$d_str="select * from info_distributor where d_id=".$n_res['d_id'];
						$d_qry=mysqli_query($conn,$d_str);
						$d_res=mysqli_fetch_assoc($d_qry);
			?>
						<li id="<?=$n_res['sale_id']?>">
							<a href="orders.php"><?=$d_res['d_agency']?> place new order</a>
							<button onclick="clearNoti(<?=$n_res['sale_id']?>)" ><img src="images/icons/close.png" /></button>
						</li>
			...
			<?php
					...
					{
						$d_str="select * from info_distributor where d_id=".$f_res['d_id'];
						$d_qry=mysqli_query($conn,$d_str);
						$d_res=mysqli_fetch_assoc($d_qry);
			?>			
						<li id="<?=$f_res['f_id']?>">
							<a href="feedback.php"><?=$d_res['d_agency']?> want to say something</a>
