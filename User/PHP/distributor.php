...
<head>
    <title>The Eatbit's-Distributor Login</title>
    ...
    <form method="POST">
        <div class="cont20">
            <div class="contact_form">
                <input type="email" id="emailBx" name="email" placeholder="Email" required>
                <input type="password" name="pwd" placeholder="Password" required>
                <button name="lgn">LOGIN</button></br>
                <a href="distributor_form.php">Want to be a distributor? Click Here!</a>
            </div>
        </div>
    </form>
    <?php
		...
		if(isset($_POST['lgn']))
		{
			$str="select * from info_distributor where d_status=1 and d_email='".$_POST['email']."'";
			...
			if(mysqli_num_rows($qry)>0)
			{
				...
				if($_POST['pwd']==$res['d_pwd'])
				{
					...
					echo "<script>window.location.href='../../distributor/PHP/index.php'</script>";
				}
				else
				{
					echo "<script>
